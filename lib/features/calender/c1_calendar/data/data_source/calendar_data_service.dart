import 'package:grc_module/features/calender/c1_calendar/data/models/enum/services_management_module.dart';
...
import 'package:grc_module/features/calender/c1_calendar/domain/enums/services_module/services_calendar_events.dart';
import 'package:grc_module/features/calender/c1_calendar/domain/enums/role_module/role_management_module/role_management_calendar_events.dart';
import 'package:grc_module/features/calender/c1_calendar/domain/enums/role_module/user_access_module/user_access_calendar_events.dart';
...
class CalendarDataService {
  ...
  CalendarDataService({required this.departmentCubit});
  ...
  Future<List<CalendarEventModel>> getRoleManagementCalendarEvents({
    ...
    try {
      ...
      for (final doc in employeeSnapshot.docs) {
        final emails = doc.data()['Email'];
        if (emails is List &&
            emails.isNotEmpty &&
            emails.last.toString().toLowerCase() ==
                currentUserEmail.toLowerCase()) {
          employeeId = doc.id;
          break;
        }
      }
      if (employeeId == null) return events;
      ...
      if (!permissionDoc.exists) return events;
      ...
      if (roles.isEmpty) return events;
      ...
      if (from != null) {
        // The access window opens: the day itself, plus the advance reminder
        // when the start is still in the future.
        ...
        if (from.isAfter(DateTime.now())) {
          add(RoleManagementCalendarEvent.accessScheduled14DaysBeforeStart, from);
        }
      ...
      debugPrint('✅ ROLE MANAGEMENT calendar events: ${events.length}');
    } catch (e) {
      debugPrint('❌ getRoleManagementCalendarEvents error: $e');
    }
    return events;
  }
  ...
  //
  //  ⚠️ Four of the eight spec entries are not emitted, because the data
  //  they need does not exist yet:
  //    • accountLocked / accountUnlocked - `Status` records the state but
  //      no timestamp, so there is no date to place a card on.
  //    • scheduledActivationDateUpdated / scheduledDeactivationDateUpdated -
  //      these two fields are single strings with no history, so a change
  //      cannot be detected. Role Management can do this because its dates
  //      ARE history lists.
  //  Give those fields a history (or a `*_Changed_At` timestamp) and the
  //  four remaining entries drop straight in.
  // ══════════════════════════════════════════════════════════════════
  Future<List<CalendarEventModel>> getUserAccessCalendarEvents({
    ...
    try {
      ...
      for (final doc in snapshot.docs) {
        ...
        if (emails.last.toLowerCase() != currentUserEmail.toLowerCase()) continue;

        final names = _historyList(data['First_Name']);
        final displayName = names.isNotEmpty ? names.last : emails.last;
        ...
        if (activation != null) {
          ...
          if (activation.isAfter(DateTime.now())) {
        ...
        if (deactivation != null) {
          ...
          if (deactivation.isAfter(DateTime.now())) {
            add(UserAccessCalendarEvent.accountDeactivationIn14Days, deactivation);
          }
        }
        break; // one employee document per signed-in user
      }

      debugPrint('✅ USER ACCESS calendar events: ${events.length}');
    } catch (e) {
      debugPrint('❌ getUserAccessCalendarEvents error: $e');
    }
    return events;
  }
        ...
        try {
          ...
          if (state.toLowerCase() == 'done') {
            if (isRequestedByUser) {
              ...
              // ✅ Wording depends on whether the SLA deadline had passed
              final doneEvent = exceededSLA
                  ? ServicesCalendarEvent.doneAfterSlaForRequester
                  : ServicesCalendarEvent.doneForRequester;
              print('   ✅ Using ${doneEvent.key} message');
