...
class _ReassignOwnerPageState extends State<ReassignOwnerPage> {
  ...
  Future<void> _submit(BuildContext context) async {
    ...
    final ownerError = _newSelectedEmployees.isEmpty
        ? S.of(context).pleaseSelectANewControlOwner
        : newOwnerEmail == widget.owner.ownerEmail
            ? S.of(context).newOwnerCannotBeTheCurrentOwner
            : null;
    ...
    if (state is GrcRequestActionSuccess) {
      showSuccessDialog(
        ...
        subtitle: S.of(context).yourReassignOwnerRequestHasBeenSubmitted,
      );
  ...
  Future<bool> _confirmReassign(BuildContext context) async {
    ...
    await showConfirmDialog(
      context: context,
      title: S.of(context).reassignOwner,
            ...
            children: [
              PaginationAppBar(
                screensTitles: [
                  ...
                  S.of(context).reassignControlOwnerRequest,
                ],
              ),
              Expanded(
  ...
  List<Widget> _buildCurrentOwnerSection(BuildContext context) {
    ...
    return [
      Text(S.of(context).currentControlOwner,
          style: StyleText.fontSize16Weight600.copyWith(color: AppColors.text)),
      SizedBox(height: 8.h),
      ContactCard(
        name: currentName,
        jobTitle:
            currentTitle.isNotEmpty ? currentTitle :grcTr(context, grcMockJobTitleFallback),
        department:
            currentDept.isNotEmpty ? currentDept :grcTr(context, grcMockDepartmentFallback),
  ...
  List<Widget> _buildNewOwnerSection() {
    return [
      Text(S.of(context).newControlOwner,
          style: StyleText.fontSize16Weight600.copyWith(color: AppColors.text)),
      SizedBox(height: 8.h),
      Container(
    ...
    return Row(
      ...
      children: [
        ...
        _submitting
            ...
            : customButton(
                width: 120.w,
                title: _submitting ? S.of(context).submitting : S.of(context).submit,
