  ...
  void _handleResize(bool isMobile) {
    ...
    if (isMobile) {
      ...
      if (Get.isRegistered<AppDrawerController>()) {
        print('🗑️ [MAIN-RESPONSIVE] Removing AppDrawerController');
        Get.delete<AppDrawerController>(force: true);
      }

      if (!Get.isRegistered<NavBarController>()) {
        print('✅ [MAIN-RESPONSIVE] Initializing NavBarController');
        Get.put(NavBarController());
      }
    } else {
      // Switched to desktop - clean up navbar, add drawer
      if (Get.isRegistered<NavBarController>()) {
        print('🗑️ [MAIN-RESPONSIVE] Removing NavBarController');
        Get.delete<NavBarController>(force: true);
      }

      if (!Get.isRegistered<AppDrawerController>()) {
        print('✅ [MAIN-RESPONSIVE] Initializing AppDrawerController');
