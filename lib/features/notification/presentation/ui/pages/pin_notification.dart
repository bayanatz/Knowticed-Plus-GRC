...
// REMOVED_MODULE: import 'package:demo_app/features/external/todo_new_module/external/tasks_module/core/custom_widgets/SideFrameMaster.dart';
import 'package:lottie/lottie.dart';
import 'package:timeago/timeago.dart' as timeago;

import 'package:demo_app/core/helper/main_helper/cross_axis_count_helper.dart';
import 'package:demo_app/generated/l10n.dart';
// REMOVED_MODULE: import 'package:demo_app/external/inventory_module/core/drop_down.dart';
import 'package:demo_app/core/theme/app_colors.dart';
import 'package:demo_app/features/employee/presentation/controller/main_core_employee_controller.dart';
// REMOVED_MODULE: import 'package:demo_app/external/services_mangment_module/Category/presentation/ui/service_department_manager/mobile/dashBoard_master_mobile/widget/dialog.dart';
// REMOVED_MODULE: import 'package:demo_app/external/services_mangment_module/core/custom_pop_up.dart';
...
class _NotificationContentState extends State<_NotificationContent> {
  ...
  List<NotificationModelSystem> _getFilteredNotifications() {
    ...
    debugPrint('📊 [PINNED PAGE] Total notifications received: ${filtered.length}');
    for (var n in filtered) {
      debugPrint('📌 [PINNED PAGE] Module: ${n.nameOfModule}, Title: ${n.title}, isPinned: ${n.isPinned}, isClean: ${n.isClean}');
    }

    // ✅ ONLY show PINNED notifications (isPinned == true)
    filtered = filtered.where((n) => n.isPinned == true).toList();
    debugPrint('📊 [PINNED PAGE] After filtering isPinned=true: ${filtered.length}');

    // Still filter out cleaned notifications
    filtered = filtered.where((n) => n.isClean == false).toList();
    debugPrint('📊 [PINNED PAGE] After removing cleaned: ${filtered.length}');

    // ✅ FIXED: Module filter with better debugging (EXACT MATCH TO MAIN PAGE)
    if (selectStatus != "All") {
      String moduleFilter = _getModuleKeyFromName(selectStatus);
      debugPrint('🔍 [PINNED PAGE] Filtering by module: $selectStatus -> $moduleFilter');

      filtered = filtered.where((n) {
        bool matches = n.nameOfModule.toLowerCase() == moduleFilter.toLowerCase();
        debugPrint('  [PINNED PAGE] Checking: ${n.nameOfModule} == $moduleFilter ? $matches');
        return matches;
      }).toList();

      debugPrint('📊 [PINNED PAGE] After module filter: ${filtered.length}');
    }
    ...
    if (searchController.text.isNotEmpty) {
      ...
      debugPrint('📊 [PINNED PAGE] After search filter: ${filtered.length}');
    }
    ...
    debugPrint('📊 [PINNED PAGE] Final filtered count: ${filtered.length}');
    return filtered;
  }

  // ✅ FIXED: Better module name matching (EXACT MATCH TO MAIN PAGE)
  String _getModuleKeyFromName(String displayName) {
    debugPrint('🔑 [PINNED PAGE] Looking for module key for: $displayName');

    for (Modules module in widget.allowedModules) {
      if (module.getModuleName == displayName) {
        String key = _moduleEnumToKey(module);
        debugPrint('✅ [PINNED PAGE] Found match: $displayName -> $key');
        return key;
      }
    }

    debugPrint('❌ [PINNED PAGE] No match found for: $displayName');
  ...
  Map<String, int> _getNotificationCountsByModule() {
    ...
    debugPrint('📊 [PINNED PAGE] Module counts: $counts');
    return counts;
  }
  ...
  Future<void> _handlePinToggle(NotificationModelSystem notification) async {
    try {
      // ✅ Set pin status to false (unpinning)
      bool newPinStatus = false;

      // Update in Firebase
      await widget.notificationService.updatePinStatus(notification.id!, newPinStatus);
      ...
      debugPrint('❌ Error unpinning notification: $e');
    }
  }

  // ✅ FIXED: Simplified navigation (EXACT MATCH TO MAIN PAGE)
  void _handleViewNotification(NotificationModelSystem notification) {
    debugPrint('🔔 [PINNED PAGE] Attempting navigation for: ${notification.nameOfPage}');
    debugPrint('📦 [PINNED PAGE] Module: ${notification.nameOfModule}');

    // Map module name to Modules enum
    Modules? targetModule = _getModuleFromString(notification.nameOfModule);

    if (targetModule == null) {
      debugPrint('❌ [PINNED PAGE] Could not find module for: ${notification.nameOfModule}');
      Get.snackbar(
        'Navigation Error',
    ...
    try {
      ...
      if (moduleIndex == -1) {
        debugPrint('❌ [PINNED PAGE] Module ${targetModule.name} not in allowed modules');
        Get.snackbar(
          'Access Denied',
      ...
      debugPrint('✅ [PINNED PAGE] Navigated to ${targetModule.name} at index $moduleIndex');

    } catch (e) {
      debugPrint('❌ [PINNED PAGE] Navigation error: $e');
      Get.snackbar(
        'Error',
  ...
  String _getModuleIcon(String moduleName) {
    ...
    return "assets/icons_assets/notification_assets/services_Info.svg";
  }
    ...
    return GestureDetector(
      ...
      child: Container(
        decoration: BoxDecoration(
          ...
          border: Border.all(color: AppColors.primary.withOpacity(0.3), width: 1.5), // ✅ Visual indicator for pinned
        ),
        child: Padding(
                      ...
                      child: Row(
                        children: [
                          ...
                          Column(
                            ...
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: [
                              Text(senderName, style: StyleText.fontSize12Weight500.copyWith(color: AppColors.text)),
                  ...
                  Column(
                    children: [
                      customButtonWithImage(
                        ...
                        padding: EdgeInsets.symmetric(horizontal: 23.5.sp),
                        height: 25.h,
                        ...
                        space: 8,
                        radius: 4.r,
                        image: "assets/icons_assets/main_icons_assets/msg_new_icon.svg",
                        widthImage: 16.sp,
                        heightImage: 16.sp,
                      ...
                      Row(
                        children: [
                          customButtonWithImage(
                            ...
                            space: 8.sp,
                            radius: 4.r,
                            image: "assets/icons_assets/notification_assets/new_pin_icon.svg",
                            widthImage: 16.sp,
                            heightImage: 16.sp,
                            colorBorder: Colors.transparent,
                          ),
                          ...
                          customButtonWithImage(
                            ...
                            space: 8.sp,
                            radius: 4.r,
                            image: "assets/icons_assets/notification_assets/new_view_icon.svg",
                            widthImage: 16.sp,
                            heightImage: 16.sp,
                            colorBorder: Colors.transparent,
                          ),
      ...
      child: Row(
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              color: light ? (isSelected ? AppColors.primary : AppColors.card) : (isSelected ? AppColors.primary : AppColors.textButton),
              borderRadius: BorderRadius.circular(4.r),
            ),
            child: Center(
              child: Text(
