...
class LoginController extends GetxController {
  ...
  void _listenToDepartments() {
    ...
    try {
      _departmentsSubscription =
          db.collection('departments').snapshots().listen((snapshot) {
        print("\n🔄 ════════════════════════════════════════");
  ...
  Future<void> getAllEmployees() async {
    ...
    print(
        "👥 Processing ${addEmployeeController.employeesWithoutFilter!.length} employees");
    ...
    try {
      _demoPermissionsSubscription = db // ← ALREADY CORRECT
        ...
        if (snapshot.exists) {
          ...
          print(
              "⚠️ Document does not exist at path: Demo_Permissions/$_companyId");
        }
      ...
      print(
          "✅ Demo_Permissions listener setup complete (using db/prod)"); // ← UPDATED
      print("✅ ════════════════════════════════════════════════════════");
    } catch (e, stackTrace) {
      print("🔴 ════════════════════════════════════════════════════════");
    ...
    try {
      _demoPermissionsSubscription = db
        ...
        if (snapshot.exists) {
          ...
          try {
            ...
            if (companyData.containsKey('modules') &&
              ...
              if (modulesMap.containsKey('modules') &&
                ...
                print(
                    "🔄 Company has ${companyModules.length} modules: $companyModules");
          ...
          print(
              "⚠️ Company document does not exist at path: Demo/$_companyId/Companys/$_companyId");
        }
  ...
  Future<void> _syncMasterAdminWithCompanyModules(
    ...
    try {
      ...
      List<RoleHistoryModel> masterAdminRoles = allRoles.where((role) {
        bool isMasterAdmin =
            role.currentRoleName.toLowerCase() == 'master admin';
        print(
            "   ${role.currentRoleName}: ${isMasterAdmin ? '✅ MASTER ADMIN' : '⏭️  Skip'}");
      ...
      if (currentModules.length != companyModules.length) {
        needsUpdate = true;
        print(
            "⚠️ Module count mismatch: ${currentModules.length} vs ${companyModules.length}");
      }
      ...
      if (needsUpdate) {
        ...
        DocumentReference roleRef =
            db.collection(getBaseUrl('Roles')).doc(roleId);
    ...
    try {
      ...
      List<RoleHistoryModel> roles = allRoles.where((role) {
        bool isMasterAdmin =
            role.currentRoleName.toLowerCase() == 'master admin';
        print(
            "   ${role.currentRoleName}: ${isMasterAdmin ? '✅ MASTER ADMIN' : '❌ Skip'}");
        return isMasterAdmin;
      }).toList();
      ...
      for (int roleIndex = 0; roleIndex < roles.length; roleIndex++) {
        ...
        for (String moduleName in demoPermissions.keys) {
          ...
          print(
              "         Permission keys count: ${modulePermissions.keys.length}");
          ...
          DocumentReference rolePermRef =
              db.collection(getBaseUrl('collectionName')).doc(roleId);
          ...
          Map<String, dynamic> currentPermissions = Map<String, dynamic>.from(
              rolePermDoc.data() as Map<String, dynamic>);
          print(
              "         Current permissions keys: ${currentPermissions.keys.length}");
          ...
          print(
              "         🔄 Processing ${modulePermissions.keys.length} permissions...");
          modulePermissions.forEach((permKey, demoValue) {
            ...
            if (newValue != currentValue) {
              ...
              print(
                  "            ✓ $permKey: $currentValue → $newValue (CHANGED)");
            }
          });
  ...
  login(BuildContext context, String email, String password) async {
    ...
    Map<String, dynamic> successAuthenticationData = result.getOrElse(() => {});
    var successType =
  ...
  void _testBothInstances() async {
    ...
    final dbDoc = await db.collection('Demo_Permissions').doc('39185362').get();
  ...
  Future<void> initData(BuildContext context, bool isTablet, String email,
    ...
    try {
      ...
      try {
        await FCMSubscriptionService.subscribeToPersonalChannel(
            userEmail: email);
      ...
      Mode.owner = employee?.role.isNotEmpty == true &&
          employee!.role.last == 'super admin';
      Mode.hr =
          employee?.role.isNotEmpty == true && employee!.role.last == 'hr';
      ...
      final bool isMobileSize = screenWidth < mobileBreakpoint;

      print("📱 [NAV] Screen dimensions: ${screenWidth}x$screenHeight");
      print("📱 [NAV] Screen width: $screenWidth");
      print("📱 [NAV] Mobile breakpoint: $mobileBreakpoint");
      print("📱 [NAV] Is mobile size: $isMobileSize");
      print(
          "📱 [NAV] Will navigate to: ${isMobileSize ? 'NavScreen (mobile)' : 'CustomDrawer (desktop/tablet)'}");
      ...
      print(
          "✅ [NAV] Navigation complete to MainResponsiveScreen (handles resize)");

      print(
          "✅ [NAV] Navigation complete to ${isMobileSize ? 'mobile' : 'desktop/tablet'} layout");
      // ✅ ═══════════════════════════════════════════════════════════════
  ...
  Future<void> _setupCompanyModulesAndSync() async {
    ...
    try {
      ...
      DocumentSnapshot companyDoc =
          await db.collection(getBaseUrl('Companys')).doc(_companyId).get();
